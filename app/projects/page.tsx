...
export default function ProjectsPage() {
  return (
    <main className="max-w-[900px] mx-auto px-10 pt-16 pb-24">
      ...
      <div className="mt-12 border border-line rounded-[10px] bg-[#1B1C22] overflow-hidden">
        <div className="flex justify-between items-center px-6 py-4 border-b border-line font-mono text-[11.5px] tracking-wide text-muted">
        ...
        <div className="hidden sm:grid grid-cols-[70px_1.6fr_1fr_1fr_70px] px-6 py-3 border-b border-line font-mono text-[10.5px] tracking-wide uppercase text-muted">
          <span>Flight</span>
        ...
        {projects.map((p) => (
          <div
            key={p.flightNo}
            className={`grid grid-cols-1 sm:grid-cols-[70px_1.6fr_1fr_1fr_70px] gap-1.5 sm:gap-0 items-center px-6 py-4 border-b border-line last:border-b-0 hover:bg-white/[0.03] transition-colors ${
              p.status === "idle" ? "opacity-45" : ""
            }`}
          >
            <div className="font-mono text-[13px] text-ink">{p.flightNo}</div>
            ...
            <div className="font-mono text-[12px] text-muted">{p.gate}</div>
            <div
            ...
            <div className="font-mono text-[12px] text-muted sm:text-right">
              {p.year}
            </div>
          </div>
        ))}

        <div className="flex justify-between items-center flex-wrap gap-3 px-6 py-4 font-mono text-[10.5px] tracking-wide text-muted">
          <div className="flex gap-[18px] flex-wrap">
            ...
            <span className="flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-muted" /> Idle /
              not started
            </span>
          </div>
